  ...
  const loadData = async () => {
    ...
    try {
      ...
      setData({
        ...
        allAttendance: payload.allAttendance || [],
              ...
              <table className="hx-table">
                ...
                <tbody>
                  {data.allAttendance.length === 0 ? (
                    <tr>
                      <td colSpan="5" className="hx-table-empty">No attendance records.</td>
                    </tr>
                  ) : (
                    data.allAttendance.map((item) => (
                      <tr key={item.id}>
                        <td>{item.studentName}</td>
                        <td>{item.studentClass}</td>
                        <td>
                          <strong>{item.subject}</strong>
                          <div className="hx-text-muted">{item.examType} · {item.examDate}</div>
                        </td>
                        <td>
                          <strong>{item.teacherName}</strong>
                          <div className="hx-text-muted">{item.teacherEmail}</div>
                        </td>
                        <td>
                          <span className={`hx-badge ${item.status === "Present" ? "hx-badge-success" : item.status === "Late" ? "hx-badge-warning" : item.status === "Excused" ? "hx-badge-info" : "hx-badge-danger"}`}>
                            {item.status}
                          </span>
                        </td>
                      </tr>
