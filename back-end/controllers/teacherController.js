  ...
  try {
    ...
    const attendanceRows = await Attendance.findAll({
      ...
      include: [
        ...
        { model: ExamSession, attributes: ["subject", "exam_type", "exam_date"] },
      ],
      order: [["updatedAt", "DESC"]],
      limit: 100,
    });
    ...
    return res.json({
      ...
      summary: {
        ...
        absencesCount: attendanceRows.filter((item) => item.status === "Absent").length,
      },
      students,
      mySessions,
      sessions: mySessions,
      ...
      recentAttendance: attendanceRows.map((item) => ({
        id: item.id,
        studentName: item.User?.name || "Student",
        studentClass: item.User?.class || "-",
        subject: item.ExamSession?.subject || "Session",
        examType: item.ExamSession?.exam_type || item.exam_type,
        date: item.ExamSession?.exam_date || item.exam_date,
        status: item.status,
      })),
      governance: {
        workflow:
